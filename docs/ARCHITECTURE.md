...
Image-Viewer 采用 **Clean Architecture（整洁架构）** 设计，将应用程序分为清晰的层次结构，每层有明确的职责和依赖关系。
...
│  │  │ Position   │  │  │ClipboardPort│ │  │ImageViewerSvc│  │
...
└── egui/
    ...
    └── widgets/
        ├── mod.rs
        ├── viewer_widget.rs    # 图像查看器组件
        └── gallery_widget.rs   # 画廊组件
```
...
pub struct EguiApp {
    service: Arc<ImageViewerService>,  // Core 服务
...
impl EguiApp {
    pub fn new(cc: &CreationContext, service: Arc<ImageViewerService>) -> Self;
...
```
src/
├── clipboard.rs        # 剪贴板操作（实现 ClipboardPort）
...
EguiApp::process_pending_files()
    │
    ├──► ImageViewerService::update_state()
    │    │
...
| `CloudSyncPort` | `core/ports/` | 云同步端口（已定义） |
