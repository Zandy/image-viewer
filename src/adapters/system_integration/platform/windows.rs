...
fn is_context_menu_registered() -> bool {
    ...
    let reg_path = format!(r"{}\.{}\shell\OpenWithOASImageViewer", REGISTRY_BASE_PATH, first_ext);
...
fn is_default_app_registered() -> bool {
    ...
    unsafe {
        match winreg::RegKey::predef(winreg::enums::HKEY_CURRENT_USER).open_subkey(reg_path) {
            Ok(key) => {
                match key.get_value::<String>("ProgId") {
...
pub fn register_context_menu() -> Result<()> {
    ...
    for ext in IMAGE_EXTENSIONS {
        let shell_path = format!(r"{}\.{}\shell\OpenWithOASImageViewer", REGISTRY_BASE_PATH, ext);
        ...
        // 创建 command 子项
        let (cmd_key, _) = hkcu.create_subkey(format!(r"{}\command", shell_path))
...
pub fn set_as_default_app() -> Result<()> {
    ...
    let (app_key, _) = hkcu.create_subkey(app_path)
        .context("创建应用程序注册表项失败")?;
    
    app_key.set_value("", &"OAS Image Viewer Application")
        .context("设置应用程序名称失败")?;
    
    // 创建 DefaultIcon
    let (icon_key, _) = hkcu.create_subkey(format!(r"{}\DefaultIcon", app_path))
        .context("创建 DefaultIcon 项失败")?;
    icon_key.set_value("", &exe_path_str.as_ref())
        .context("设置图标路径失败")?;
    
    // 创建 shell\open\command
    let (cmd_key, _) = hkcu.create_subkey(format!(r"{}\shell\open\command", app_path))
    ...
    for ext in IMAGE_EXTENSIONS {
        ...
        // 设置打开方式
        ext_key.set_value("", &"OASImageViewer.Application")
